...
def login(driver):
    ...
    print("> waiting for the page to load (5 seconds)")
    time.sleep(5)
...
def main(debug=True):
    ...
    for index ,link in enumerate(distinct_links):
        ...
        POSTS.append(get_comments_per_post(driver=driver,post_URL=link))
...
def test():
    ...
    link = distinct_links[2]

    print(1,'/',len(distinct_links),'>',link)
    POSTS.append(get_comments_per_post(driver=driver,post_URL=link))
